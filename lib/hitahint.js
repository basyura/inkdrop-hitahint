...
class HitaHint {
  ...
  show = () => {
    ...
    //const len = this.hintcharacters.length;
    ...
    */
    ...
    const elements = previewPane.querySelectorAll("a");
    elements.forEach((a) => {
      let mark = "";
      if (elements.length <= this.hintcharacters.length) {
        ...
        if (pos < this.hintcharacters.length) {
          ...
          mark = this.hintcharacters[
            parseInt(pos / this.hintcharacters.length, 10) - 1
          ];
        }
        mark += this.hintcharacters[
