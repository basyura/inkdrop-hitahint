...
class HitaHint {
  ...
  extractFromSideBar = () => {
    ...
    // todo
    let all = this.extractTargets(bar, ".sidebar-menu-item-all-notes");
    if (all.length != 0) {
      all = [all[0]];
    }
    const contents = this.extractTargets(bar, ".sidebar-menu-book-list-item .content");
    // todo
    const backs = []; //this.extractTargets(bar, ".back-button");
    const tags = this.extractTargets(bar, ".sidebar-menu-tag-list-item .content");
    const links = all.concat(contents).concat(backs).concat(tags);
