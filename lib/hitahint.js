...
class HitaHint {
  ...
  openUri = (hint) => {
    ...
    try {
      if (hint.type == "link" || hint.type == "cm-link" || hint.type == "short-link-mark") {
        openLink(hint);
  ...
  createHint = (pane, ele, text) => {
    ...
    if (ele.className == "content") {
      span.type = "notebook";
    } else if (ele.className == "short-link-mark") {
      span.type = "short-link-mark";
      span.url = ele.url;
    } else if (ele.classList.contains("cm-url")) {
      span.type = "cm-link";
      span.url = ele.innerText;
